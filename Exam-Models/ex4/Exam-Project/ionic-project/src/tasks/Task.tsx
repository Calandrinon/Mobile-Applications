...
const Task: React.FC<TaskPropsExt> = ({_id, text, status, version, onEdit, token}) => {
    ...
    return (
        <IonItem>
            <IonLabel onClick={() => onEdit(_id)}>{text} {status} {version}</IonLabel>

            <IonModal isOpen={showModal} backdropDismiss={false} enterAnimation={enterAnimation}
                      leaveAnimation={leaveAnimation} cssClass='my-custom-class'>
                <IonGrid>
                    <IonRow>
            ...
            <IonButton color="success" onClick={() => deleteTask(token, String(_id))}>
