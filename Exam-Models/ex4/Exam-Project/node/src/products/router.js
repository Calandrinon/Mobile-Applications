...
router.post('/synchronize', async (ctx) => {
    ...
    let currentlySavedItems = await productStore.getAll(offlineItems[0].userId);

    for (let item of offlineItems) {
        if ("_id" in item) {
            await productStore.update({_id: item._id}, item);
        } else {
            await productStore.insert(item);
        }
    }

    response.body = await productStore.getAll(offlineItems[0].userId);
    response.status = 200;
});
